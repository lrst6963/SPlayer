// MPRIS (Media Player Remote Interfacing Specification) 接口实现
// 这是一个为 SPlayer 提供 Linux 系统媒体控制集成的 Rust 原生模块
// 通过 D-Bus 暴露标准 MPRIS 接口，使系统媒体键和桌面环境能够控制播放器
...
/// MPRIS 事件结构体
/// 当系统通过 MPRIS 控制播放器时（如按下媒体键），会触发这些事件回调到 JS 层
#[napi(object)]
#[derive(Clone)]
